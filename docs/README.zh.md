...
v2 构建链采用 source-first 与评审门禁。八个领域和 41 个模块共享同一递归概念层级；
标量字段、受控值、实例、正反例、来源、映射和验证约束均留在对应节点或关系中。
...
`releasable=true`，才能声明 v2 已发布；不得根据 candidate 构建目录推断发布状态。
