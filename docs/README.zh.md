<div align="center">
  ...
  <p>
    面向智能体系统的证据约束型本体工程框架：
    上下文输入、控制编排、运行轨迹、互操作适配、能力调用、信任与安全、可观测反馈、记忆持久化、结构化模式与图谱浏览。
  </p>
...
| 运行关注域 | 范围 |
|---|---|
| 上下文输入域 | 提示、消息、指令、文档、命令输出、来源引用、披露窗口、存储输入和进入 agent 系统的可索引上下文。 |
| 控制与编排域 | 目标、任务、计划、委派、路由、闸门、提示链、并行、投票、评估循环和综合控制。 |
| 运行状态与轨迹域 | 运行会话、参与者、环境、预算、转录、轨迹事件、检查点、快照、状态差异和运行产物。 |
| 互操作适配域 | MCP、A2A、框架、基准、状态图、模式剖面、语义导出和图视图映射；这些映射不得重定义核心术语。 |
| 能力与资源调用域 | 工具与能力注册表、定义、发现、匹配、调用、执行、MCP 传输、资源、提示、结果和可观测副作用。 |
| 信任、策略与安全域 | 信任边界、权限范围、权限提示、策略决策、沙箱、网络控制、注入防御、提交闸门、脱敏和审计披露。 |
