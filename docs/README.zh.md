...
本体领域语义只在 `ontology/source/**` 中编辑；规范 JSON、Markdown、根 Schema、
TypeScript 类型、来源索引、定义对账表与 payload 样例均由构建器确定性生成。

Moonweave 对外呈现一张 canonical Agent Ontology 图谱。定义、结构约束、实例、
正反例、来源声明、映射和治理信息均附着在对应节点或关系上，不另建平行本体或浏览页面。
...
发布规模以 `ontology/agent-ontology.json` 中构建器生成的 `ontology_metrics` 为准，
说明文档不再复制容易漂移的数字。新指标分别统计概念、`is_a`、语义关系、结构字段、
受控值、实例说明、约束、来源声明和案例路径，不再把旧版平行集合当成本体层级。
...
| 可观测性与反馈域 | 从原始运行证据派生警告、错误、诊断、遥测、日志、审计、轨迹导出、审查发现、修正、通用指标与评估运行、恢复动作和学习信号，并通过明确反馈流关系回到编排、记忆、工具选择、策略和优化循环；基准专属语义保留在适配器层。 |
| 记忆与上下文持久化域 | 作用域化记忆库、记忆记录与类型体系、摄入、分块、嵌入与索引、检索与排序、上下文组装、摘要、偏好记忆，以及写入、更新、删除、合并、巩固、过期、验证、反思、审计和记忆投毒控制等生命周期操作。 |

## 层边界

Core/Profile/Adapter 只表达归属与适用性，不形成第二套导航层级；浏览器唯一主层级是
“Agent Ontology → 领域 → 模块 → 概念 → 更具体概念 → …”。领域和模块是稳定入口，
不是概念纵深的上限。
...
构建未发布候选、落盘评审通过的正式工件树，并检查生成物漂移。仓库发布还必须通过
运行器相关的已审视觉基线门禁，并把工件与基线放入同一个提交：
...
`npm run ontology:expand` 仅作为旧命令兼容入口，会提示弃用并转调 source-first builder。
...
- `docs/rfcs/0005-unified-concept-hierarchy-and-node-information.md`：单一图谱层级与信息内嵌合同；
- `docs/governance/v1-to-v2-canonical-migration-guide.md`：v1 到 v2 的字段、消费者与回滚迁移说明。

## 当前状态

v3 构建链采用 source-first 与评审门禁。八个领域和由 `ontology_metrics.modules` 报告的已接受能力边界模块共享同一
canonical 图；概念可沿经审查的分类或结构骨架任意向下展开，标量字段、受控值、实例、正反例、
来源、映射和验证约束均留在对应节点或关系中。
Graph IR、payload Schema、fixtures 与后续语义导出只是生成投影，不是可编辑事实源。

Explorer 继续只呈现这一张图，并采用受 Graphify 启发、随应用本地打包的 `vis-network`
`ForceAtlas2Based` 力导向视图。离线 NetworkX `MultiDiGraph` 完整保留定向、多谓词、多重关系；
由它生成的简单无向投影仅用于确定性的 Leiden/Louvain 社区发现。社区颜色和枢纽节点大小只是
视觉提示，不代表本体类型、层级或业务重要性。页面在初次稳定化后冻结物理模拟；搜索、社区筛选、
聚焦、平移、缩放和拖动都发生在同一张图上，不再提供层级/关系双模式或自上而下/自左向右控件。
生产构建通过 build manifest 把部署提交与 canonical/source 指纹绑定。

只要工件仍标记为 `artifact_metadata.release_channel="candidate"` 或
`releasable=false`，它就是未发布的验收候选，即使内部 `status` 已为 `accepted` 也不代表
正式发布。只有根 canonical 及全部生成物完成同一原子切换，且根工件标记为 `release`、
`releasable=true`，才能声明 v3 已发布；不得根据 candidate 构建目录推断发布状态。
...
界面保持 Moonweave 的月色研究室视觉语言与接近 FIBO 的递归浏览结构，但始终只有一张图
和一张同页节点/关系特征表；不存在平行的 Schema、ABox、TBox、Instance、证据或适配器页面。
