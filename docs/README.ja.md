...
エクスプローラーの唯一の主階層は「Agent Ontology → ドメイン → モジュール → 概念」です。
...
未公開候補を構築し、レビュー済み成果物を公開し、生成物のドリフトを検査します。
...
v2 ビルドパイプラインは source-first とレビューゲートを採用します。八ドメインと 41 モジュールは一つの
再帰的概念階層を共有し、フィールド、統制値、例、出典、写像、検証制約は対応ノードまたは関係に残ります。
...
内部 `status` が `accepted` でも未公開の検証候補です。v2 の正式公開は、ルート canonical と
