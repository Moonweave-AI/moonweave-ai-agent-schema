...
Moonweave Agent Schema は、コンテキスト入力、制御オーケストレーション、実行、相互運用アダプテーション、能力呼び出し、信頼と安全の仲介、観測可能なフィードバック、記憶永続化という八つの運用関心ドメインでエージェントシステムを整理します。

| 運用関心ドメイン | 範囲 |
|---|---|
| コンテキスト入力ドメイン | プロンプト、メッセージ、指示、文書、コマンド出力、出典参照、開示ウィンドウ、保存入力、エージェントシステムへ入る索引可能なコンテキスト。 |
| 制御とオーケストレーションドメイン | 目標、タスク、計画、委任、ルーティング、ゲート、プロンプトチェーン、並列化、投票、評価ループ、統合制御。 |
| 実行状態とトレースドメイン | 実行セッション、参加者、環境、予算、トランスクリプト、トレースイベント、チェックポイント、スナップショット、状態差分、実行成果物。 |
| 相互運用とアダプタドメイン | MCP、A2A、フレームワーク、ベンチマーク、状態図、スキーマプロファイル、意味エクスポート、グラフビュー写像。これらはコア用語を再定義しません。 |
| 能力とリソース呼び出しドメイン | ツールと能力の登録簿、定義、発見、照合、呼び出し、実行、MCP転送、リソース、プロンプト、結果、観測可能な副作用。 |
| 信頼、ポリシー、安全ドメイン | 信頼境界、権限範囲、権限プロンプト、ポリシー判断、サンドボックス、ネットワーク制御、注入防御、コミットゲート、秘匿化、監査開示。 |
