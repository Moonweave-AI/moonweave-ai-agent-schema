...
> **最终可视化校正（2026-07-15，优先级高于本文全部旧图谱章节）：** 本计划关于模块边界、任意语义纵深、关系治理、节点内信息和单一 canonical 图的内容合同继续有效；所有关于 Cytoscape.js、ELK Layered、fCoSE、层级/关系双模式、上下/左右方向切换、按方向展开以及布局 Worker 的设计与验收条款均已撤回，不再是现行实施依据。旧段落保留仅用于解释二次升级的历史决策过程，不得据此恢复旧依赖、旧控件或旧测试。
>
> 现行图谱合同只有一种呈现：以 [Graphify](https://github.com/Graphify-Labs/graphify) 的清爽力导向网络为基线，在浏览器使用本地打包的 `vis-network` 和 `ForceAtlas2Based` 完成一次稳定化布局，稳定后冻结物理模拟。界面不提供“自上而下/自左向右”、层级模式、关系模式或布局算法选择；搜索、社区筛选、聚焦、平移、缩放、拖动和适配画布只是同一张图上的浏览动作，不产生第二套图数据。
>
> 数据职责严格分离但不分裂语义：Python/NetworkX `MultiDiGraph` 保存完整的定向、多谓词、多重关系；供 Leiden（可用时优先）或 Louvain 社区发现使用的是由它确定性生成的简单无向投影。无向投影只回答“哪些节点在视觉上更适合聚在一起”，绝不改写 canonical 方向、谓词、父子结构或归属。社区颜色、社区名称和枢纽大小都是视觉提示，不是本体层级、节点类型或业务重要性。浏览器仍绘制完整 canonical 节点和定向关系，边标签默认隐藏、悬停时显示，明确事实与派生投影以实/虚线、粗细和透明度区分。
>
> 定义、schema、字段、约束、实例、正反例、案例路径、来源、映射和治理记录继续作为对应节点或关系的同页信息；它们不得成为社区计算用的影子节点、额外 ABox/TBox/Schema/Instance 图、独立详情页面或第二套关系。现行实现与验收应检查“完整语义不丢失、社区投影不污染本体、稳定后不持续抖动、标签不过载、节点选择可抵达原有详细信息”，而不再检查层级方向、ELK bend point、fCoSE 自由度或双模式切换。

> 状态：`Implemented；可视化章节已按最终校正收口`
> 计划版本：`1.1.0`
> 编制日期：2026-07-14
...
> **阅读提示：** 以下正文按 2026-07-14 的二次升级决策原样保留，用于追溯语义整改和旧可视化方案的验收逻辑。其中本体内容条款仍可直接执行；凡涉及布局引擎、图谱模式、方向控件、Worker、边路由或前端性能预算的段落，一律先应用本页顶部最终校正，不得孤立引用。
...
`add`、`keep`、`reparent`、`retype`、`move_owner`、`convert_to_field`、`convert_to_controlled_value`、`merge`、`split`、`deprecate`、`remove_invalid`。

实施校正（2026-07-15）：本节同时要求账本覆盖“本轮人工批准的新 Concept”，但原枚举没有能真实表达“v2 中不存在、v3 经审查新增”的决策；把这类记录写成 `keep` 或伪装成 `split` 都会篡改迁移历史。因此新增最小决策 `add`：仅当冻结 v2 语义基线中没有该 `concept_id`、当前 source 中存在完整审查记录时使用；其 `current_*` 列必须为空，`required_relation_changes` 必须明确没有 v2 关系可迁移。既有 Concept 不得使用 `add`。
...
实施校正（2026-07-15）：表中的 `bezier` 是“没有 Worker 避碰路由数据时”的默认语义边样式；一旦 Worker 为 semantic cross-link 生成了经诊断的 bend points，实际画布必须改用低透明度细 `segments`，并固定 `edge-distances: node-position`。这是为了让 Cytoscape 使用的权重/距离参考线与本项目按节点中心计算的路由完全一致，使画面几何与 crossing/collision 诊断检查同一条折线。继续强制 `bezier` 会要求按 Cytoscape 多段 quadratic 规则重建并采样另一套曲线，反而再次造成“诊断通过但画面相交”。该校正只改变路由后的绘制几何，不改变 semantic relation 的本体层级、默认可见性或信息归属；依据见 [Cytoscape.js Segments edges](https://js.cytoscape.org/#style/segments-edges) 对 `edge-distances` 的定义。
...
- `normalizeElkPositions(result, direction)`，并将所有权/主骨架边按布局方向重路由为彼此独立的扇出折线，避免 ELK 正交共享干线造成边重叠、穿越节点或标签碰撞；
...
实施校正（2026-07-15）：这里的“回退 breadthfirst”指按经审查逻辑深度执行的确定性广度分层，最终实现为 `fallbackLayeredPositions()`，而不是再次调用 Cytoscape breadthfirst 插件。该实现沿用与 Worker 相同的投影、方向和节点尺寸输入，可在 Worker 不可用、响应非法或超时时立即产生可诊断坐标，并继续接受 120/220 场景预算与布局 invariant 检查；因此没有在主线程启动第二个不受控布局器。关系模式的 fCoSE 若同步抛错、发出 `layouterror` 或超过 6.5 秒，则先恢复同结构的最近稳定层级快照；没有可兼容快照时才使用这一确定性分层回退。
...
`commit_sha,source_fingerprint,canonical_fingerprint,generator_version,built_from_ref,module_count,concept_count,relation_count`。其中 `relation_count` 只统计 `status = accepted` 的当前有效关系；`deprecated` 关系记录继续保留在 canonical artifact 中用于迁移谱系与可追溯性，但不得计入当前图谱身份。
...
- [ ] 所有 accepted Concept 有自然三语定义、正例、边界例和实际来源；deprecated Concept 保留完整替代与弃用谱系，且不再发布现行例子；
...
> Moonweave 保留 FIBO 式的稳定领域入口和单一图谱，但不再把“Domain—Module—Concept”当作三层上限，也不再把共享词头概念机械平铺。模块边界由问题和 owner 决定，概念纵深由真实分类与结构关系决定；前端以 Graphify 风格的单一 `vis-network` `ForceAtlas2Based` 网络清楚呈现完整关系，以 NetworkX 无向投影仅辅助社区着色；所有解释仍回到同一节点和同一关系。
